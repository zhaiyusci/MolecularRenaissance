...
(function (root) {
  ...
  const examples={
    sphere:{name:'单球 · 排线研究',atoms:[atom('C',0,0,0)],bonds:[]},
  ...
  function elementColor(element,strength=.65,saturation=1){
    const hex=elementPalette[element]||'#ded8cf';
  ...
  function render(molecule,options={}){
    ...
    for(const k of ['outlineWidth','hatchWidth','colorSaturation','washOffsetX','washOffsetY','width','height','yaw','pitch','lightAzimuth','lightElevation','lightDistance','density','lineWidth','labelSize','labelStrokeWidth','washStrength']) if(!Number.isFinite(o[k])) throw new Error('Invalid option: '+k);
    ...
    function curve(fn,steps,width,accept=()=>true,engrave=false,closed=false){
      if(width===0)return; // Do not emit degenerate ribbons or SVG hairlines.
      let run=[];
      const runs=[];
      const flush=()=>{if(run.length>1)runs.push(run);run=[];};
      for(let i=0;i<=steps;i++){
        const {p,n}=fn(i/steps),lit=illumination(n,p);
    ...
    for(const s of spheres){
      ...
      function hatch(axis,count,secondary){
        ...
        for(let j=1;j<count;j++){
          ...
          },Math.max(120,Math.ceil(2*Math.PI*s.r*scale*r/.7)),o.hatchWidth*(secondary?.63:.8),(n,p,lit)=>n[2]>=0 && lit<(secondary?.12:(j%2===0?.88:.58)),true,true);
        }
      }
    ...
    const fillFor=element=>o.colorWash?elementColor(element,o.washStrength,o.colorSaturation):'#ffffff';
    let wash='';
    ...
    return `<svg xmlns="http://www.w3.org/2000/svg" width="${o.width}" height="${o.height}" viewBox="0 0 ${o.width} ${o.height}" role="img" aria-labelledby="title"><title id="title">${esc(molecule.name||'Molecular engraving')}</title><rect width="100%" height="100%" fill="white"/>${wash}<g data-role="engraving" fill="none" stroke="#161616" stroke-linecap="round" stroke-linejoin="round">${paths.join('')}</g><g font-family="Georgia, 'Times New Roman', serif">${labels}<text x="${o.width/2}" y="${o.height-53}" text-anchor="middle" fill="#161616" font-size="19">${esc(molecule.name||'Molecular engraving')}</text><text x="${o.width/2}" y="${o.height-28}" text-anchor="middle" fill="#555" font-size="10" letter-spacing="2">ORTHOGRAPHIC • LINE ENGRAVING</text></g></svg>`;
  }
  const api={render,examples,depthAt,engravingWidth,elementColor,elementPalette};
