...
(function (root, factory) {
  ...
  else root.MolWash = factory();
}(typeof globalThis !== 'undefined' ? globalThis : this, function () {
  'use strict';

  /**
   * buildWash(scene, depthAt, project, scale, colorForElement) -> SVG <g> string.
   ...
   */
  function buildWash(scene, depthAt, project, scale, colorForElement) {
    ...
    function chain(nodes, pair) {
      ...
      if (!curves) {
        ...
        curves = fitContour(samples, 0.012); fitted.set(key, curves);
      }
      ...
      for (var iteration = 0; iteration < 5; iteration++) {
        ...
        for (var j = 1; j < count; j++) {
          ...
          for (var axis = 0; axis < 2; axis++) {
            ...
            dd[axis] = 6 * (s * (curve[2][axis] - 2 * curve[1][axis] + curve[0][axis]) + t * (curve[3][axis] - 2 * curve[2][axis] + curve[1][axis]));
          }
          var r = [q[0] - p[lo + j][0], q[1] - p[lo + j][1]], denominator = dot(d, d) + dot(r, dd);
  ...
  return { buildWash: buildWash, fitContour: fitContour };
}));
