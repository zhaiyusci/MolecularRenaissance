...
TypeScript and Rollup are development dependencies only. `strict` is enabled, and the core compiles without DOM type libraries. `npm test` automatically builds first, then runs the gate lane: every script in it must stay in the seconds range, because a gate nobody is willing to wait for stops protecting the tree. `npm run test:heavy` runs the exhaustive sweeps (rotation tables, tone and numeric sweeps) that are too slow for that lane, and `npm run test:all` runs both. The migration also retains SHA-256 baselines for 49 sets of original JS output, using explicit historical-radius and fixed-scale fixtures to check algorithmic consistency in CommonJS, ES Module, and DOM-free classic-script environments. Current scientific radii and fixed scale have separate tests.
...
## Shading Quantization Levels

The GUI always selects quantized hatching, stippling, and halftone in both precise and fast-overlay modes. **Shading levels** offers **4, 8, 16, 32, or 64**, defaulting and resetting to **16**. Switching texture or fast overlay preserves the selected level count. Disabling shading only disables the control; preview and SVG export use the same selection.

The former “16-level shading” checkbox has been removed: continuous individual-dot/hatch rendering is no longer a GUI choice. Backend routes such as `quantizeShading: false` and `stippleFill: 'marks'` remain available. The new `shadingLevels` option accepts only the five numeric values above. Historical references and measurements for 16 levels below describe the default count, not measurements of every count.
...
render(molecule, {renderMode: 'fast', castShadows: false,
  quantizeShading: true, shadingLevels: 16});
// renderMode: 'precise' (default) retains the original renderer.
```

The projection is already orthographic. Fast mode does not compute sphere–sphere intersection curves: complete atoms and complete labels are overlaid in sphere-center depth order, without clipping labels or checking whether their centers are visible. With directional lighting, each distinct sphere radius generates one complete local shading template, translated through SVG `use` for hatching, stippling, and halftone alike. Tone regions and clips are shared too, not merely hatch skeletons or PNG images. Different radii get separate templates rather than scaling texture thickness. Per-atom fills, element patterns, outlines, labels, and painter ordering retain their own styling. The presence of bonds does not trigger a scene-wide fallback: bonds completely buried within their endpoint atoms are identified geometrically, while the remaining bonds use local depth tests against nearby atoms to generate vector visibility regions. Coordinates, physical radii, and fixed scale remain unchanged, but overlapping spheres meet in illustrative layers rather than precise three-dimensional intersection surfaces; bond–bond visibility is also handled only by sorting.

The API explicitly errors on fast mode combined with cast shadows (`Fast rendering requires castShadows=false`), rather than silently falling back. Local bond regions use discovery steps of 0.5 SVG units for preview and 0.25 for export. Discovered boundaries are refined by bisection, but sub-grid fragments may still be missed. GUI stippling uses the selected quantized bitmap tile palette in fast mode too. Templates use atom-local texture coordinates: equal-radius copies repeat dot positions/screen phase instead of resampling world coordinates for every atom. This is an intentional fast-copy tradeoff. The cache is shared only within a render, so lighting, levels, scale, brightness, and contrast edits generate fresh templates. Bonds still require their own directional textures and local visibility, but only bonds that pass visibility checks generate shading.

**Historical measurements (older radii and fast textures, not the current defaults):** median SVG generation times in that round for the actual C60 model (including all 90 bonds): default-radius hatching **209.7 → 5.0 ms**; a fixture with atoms scaled to 0.65 to expose bonds **238.1 → 70.2 ms**. Default-radius stippling was **209.4 → 170.5 ms** and halftone **72.7 → 49.0 ms**; with smaller atoms, stippling instead worsened from **222.0 → 279.3 ms**, so not every mode is faster. All default C60 bonds lie within the union of their endpoint atoms; the reduced-radius fixture has 74 visible bonds, with none removed. Timings exclude browser painting; raw data and parameters are in `benchmarks/fast-renderer.json`.

**Current atom-template comparison** (scientific radii at 0.75×, 16 levels, actual C60 retaining all 90 bonds, 61 visible): hatch **85.6 → 71.8 ms**, stipple **129.2 → 87.2 ms**, halftone **110.6 → 78.3 ms**; SVG size falls by about **51%, 42%, and 44%** respectively. All three use **1 atom template and 60 instances**. A separate atoms-only fixture isolates atom work: 9.2/41.6/20.8 ms becomes 1.2/1.4/1.3 ms; these are not complete-C60 timings, whose remaining work is primarily local bond visibility. Results are medians of five alternating same-process pairs, SVG generation only, not browser paint. Raw results: `benchmarks/fast-templates.json`; script: `scripts/bench-fast-templates.cjs` (requires capturing the previous bundles in the documented snapshot directory before changing/building).

Checks: `node test-fast-templates.cjs`, `node test-fast-renderer.mjs`, `node test-fast-ui.cjs`. Historical precise/fast comparison: `node scripts/bench-fast.mjs`. The earlier certified prototype in `PAINTER-EXPERIMENT.md` no longer represents the capabilities of the main page's fast toggle.
...
The pattern-based halftone descriptions below refer to `quantizeShading: true` (the precise-mode default). With `false`, halftone outputs individual dots with continuous radii on the regular 45° grid; whole-dot edge contraction remains, so boundary coverage differs from clipped patterns. Per-dot output costs and limits apply. For precise stippling, `true` snaps local tone to the same 16 shared levels while leaving the dot radius at `dotSize`, so only the number of marks changes; `false` retains the original continuous-density stipple. `stippleFill` chooses how that 16-level ink is delivered: `'marks'` emits flat, radius-and-level batched round-cap subpaths with no texture tile and no per-owner clip, while `'bitmap'` (the API and GUI default) paints baked 1-bit PNG tiles behind one shared pattern transform, so the browser blits a decoded image instead of stroking the mark set. The bitmap path needs certified owner clips, falls back to `'marks'` without them, and is larger on the smallest models because its tile payload is a fixed cost.
...
- **Lighting precedes artistic processing**: direct illumination is `D = max(0, n·L)`, with cast shadows multiplying only D by `1 - shadowStrength`. No bounce or ambient illumination is simulated. Only afterward apply the artistic baseline `B = (D + 1) / 2`, brightness/contrast and texture mapping, then quantization. Artistic processing may lift physically unlit regions without pretending to add ambient light. Shadow attenuation no longer acts on this artistic offset, eliminating the false bright rim at the light-facing/back-facing transition.
- `shadingBrightness` ranges from −1–1, defaults to 0, and brightens with positive values; `shadingContrast` adjusts the tonal response. Both operate after lighting and neither changes element-colored fills. Controls/defaults are unchanged; corrected shadow/back-facing output intentionally differs from the old lighting formula.
...
- The GUI always sends `quantizeShading: true` for precise and fast rendering, with `shadingLevels` selected from 4/8/16/32/64 (default 16); it never emits `hatchMode`. Quantized hatching clips fixed curved skeletons to the selected number of tone bands; widths are quantized and cut ends do not taper. Halftone coverage and stipple density use the same level count; bitmap stippling uses matching pre-baked tiles. Backend-only `quantizeShading: false` retains continuous tapered hatching, continuously sized halftone dots, and continuous-density stippling. Uncertifiable precise hatch geometry retains the internal `continuous-fallback` safety path; removing its GUI selector does not remove that geometric fallback. These are not pixel-equivalent replacements for quantized patterns. See [the implementation and historical measurements](LAYERED-HATCHING.md).
- Legacy `hatchMode: 'layered' | 'continuous'` remains a hatch-only compatibility option. Conflicting explicit selectors are rejected, as is nonboolean `quantizeShading`. Fast mode now accepts explicit `quantizeShading: true` for quantized shading and `false` for the retained continuous routes. Omitting it keeps legacy fast behavior; GUI callers always pass `true`. Historical performance and appearance approvals describe their recorded configurations, not all new level counts.
