...
标签随所属原子一起按中心深度由后往前绘制，不再作为最上层文字叠加。每个表面依次绘制不透明底色、纹理／轮廓及自己的标签，前景表面自然盖住后景文字和描边；关闭底色时仍用不透明白色完成遮挡。文字本身及其容器不加裁剪，保留完整、可编辑的字形。三种纹理都按所属表面输出，不通过重复整个场景实现覆盖；关闭标签时保持原输出路径。无法认证可见区域的特殊模型暂不显示标签，而非浮盖在几何上。验证：`node test-label-depth.cjs`。
