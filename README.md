...
const svg = render(examples.ethanol, {
  ...
  quantizeShading: true, // Precise hatch/halftone default; false selects continuous shading
...
- Precise rendering uses shared `quantizeShading: true` by default (omitted/`undefined`). The **16-level shading** checkbox (中文：**16级明暗**; formerly **16-level hatching**) controls both hatching and regular halftone, starts checked, and Reset restores it. It is disabled when shading is off, in fast mode, or for stippling, without clearing either preference; it remains enabled for precise halftone. The UI emits the boolean only for precise hatch/halftone, omits it for fast/stipple, and never emits `hatchMode`. Quantized hatching reuses fixed curved skeletons with 16 disjoint tone clips; equal-radius spheres share definitions. Widths are quantized and cut ends do not taper. With `false`, hatch uses legacy continuous tapered ribbons; halftone uses individual continuously sized dots on a 45° regular grid, contracting edge dots to fit whole marks rather than clipping patterns. This is not a pixel-equivalent replacement for quantized patterns. Stippling ignores the switch. Unsupported hatch geometry still reports `data-hatch-mode="continuous-fallback"`. See [the implementation and historical measurements](LAYERED-HATCHING.md).
