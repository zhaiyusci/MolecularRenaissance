...
The pattern-based halftone descriptions below refer to `quantizeShading: true` (the precise-mode default). With `false`, halftone outputs individual dots with continuous radii on the regular 45° grid; whole-dot edge contraction remains, so boundary coverage differs from clipped patterns. Per-dot output costs and limits apply. For precise stippling, `true` snaps local tone to the same 16 shared levels while leaving the dot radius at `dotSize`, so only the number of marks changes; `false` retains the original continuous-density stipple. `stippleFill` chooses how that 16-level ink is delivered: `'marks'` (default) emits flat, radius-and-level batched round-cap subpaths with no texture tile and no per-owner clip, while `'bitmap'` paints baked 1-bit PNG tiles behind one shared pattern transform, so the browser blits a decoded image instead of stroking the mark set. The bitmap path needs certified owner clips, falls back to `'marks'` without them, and is larger on the smallest models because its tile payload is a fixed cost.
