...
渲染核心使用 **TypeScript**，运行时不依赖 DOM 或第三方库。仓库附带编译好的 JavaScript，直接打开页面不需要构建。白底黑线，可使用浓度与饱和度可调的元素底色或彩色纹理；不使用纸张纹理、噪声、褪色或灰度渐变。
...
底色、彩色纹理、黑色明暗和轮廓均保持原始几何位置，不再做色层平移。旧脚本传入的偏移参数不再生效。关闭元素上色时禁用上色方式、浓度和饱和度控件但保留值；重置恢复 `wash` 与默认浓度等设置。
...
- 常见球棍模型直接查共享可见区域，按到边界的保守距离缩小圆点，不再绕圆做遮挡射线测试。复杂场景回退才使用旧的 16 个方向、两圈 footprint 测试和二分半径缩小。输出始终是 SVG `<circle>`，不是将圆点剪成多段路径。
- 轮廓始终独立，由 `outlineWidth` 控制；填色、光源和标签保持可用。三种模式始终启用同一组密度、大小、对比度；切换模式不会重置这三个值。仅交叉排线和渐变线宽在网点模式禁用并保留其值。所有控件沿用同一个 requestAnimationFrame 渲染调度。
