...
function geometry(svg){const ids=[...new Set(svg.match(/\blh-[a-z0-9]+-[a-z0-9]+(?=-)/g)||[])];assert(ids.length<=1);if(ids.length)svg=svg.split(ids[0]).join('lh-NAMESPACE');return svg.replace(/<text\b[^>]*>[\s\S]*?<\/text>/g,'').replace(/fast-painter-[0-9a-f]+-[0-9a-f]+/g,'fast-id').replace(/data-whole-labels="\d+"/g,'data-whole-labels="ignored"');}
let cases=0;
