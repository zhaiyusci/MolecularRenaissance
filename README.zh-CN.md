...
**原子球大小另由用户控制**：`atomRadiusScale`（API 默认 1，GUI 范围 0.2–2、初值 0.75）只乘球半径，不改变原子中心距离、键长或键柱粗细；与整体投影 `scale` 独立，也不会自动选择倍率。可用 `atom.radius` 明确覆盖基准半径；未知元素不再猜用碳半径。真实半径引起的普通球球相交现由解析交圆处理，不擅自缩小原子或改变键长来回避。来源、适用范围、C(sp²)/C(sp) 以及验证见 [RADII.md](RADII.md)。后文早期性能表/优化百分比为旧绘图半径下的历史记录，不代表当前默认画幅与半径的测量结果。
