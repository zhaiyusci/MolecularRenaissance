...
**原子球大小另由用户控制**：`atomRadiusScale`（默认 0.75，GUI 范围 0.2–2）只乘球半径，不改变原子中心距离、键长或键柱粗细；与整体投影 `scale` 独立，也不会自动选择倍率。可用 `atom.radius` 明确覆盖基准半径；未知元素不再猜用碳半径。真实半径引起的普通球球相交现由解析交圆处理，不擅自缩小原子或改变键长来回避。来源、适用范围、C(sp²)/C(sp) 以及验证见 [RADII.md](RADII.md)。后文早期性能表/优化百分比为旧绘图半径下的历史记录，不代表当前默认画幅与半径的测量结果。
...
TypeScript 和 Rollup 仅为开发依赖；`strict` 开启，核心编译不引入 DOM 类型库。`npm test` 自动先构建，再运行**闸门道**：其中的每个脚本都必须保持在秒级，因为没人愿意等的闸门等于不再保护仓库。`npm run test:heavy` 运行不适合该道的穷举扫掠（旋转表、色调与数值扫掠），`npm run test:all` 两道都跑。迁移还保留了 49 组原 JS 输出的 SHA-256 基线，通过显式历史半径与固定比例夹具检查 CommonJS、ES Module 和无 DOM 经典脚本环境中的算法一致性；当前科学半径与固定比例另有独立测试。
...
GUI 的排线、点刻、规则网点始终选择量化路线，精确模式和快速覆盖均适用。「明暗级数」可选 **4、8、16、32、64**，默认和重置值为 **16**；切换纹理或快速覆盖保留级数。关闭光影时仅禁用控件，不清空选择。预览与 SVG 下载使用同一级数。

原「16级明暗」复选框已移除，GUI 不再提供连续逐点／连续排线入口。后端仍保留 `quantizeShading: false` 与 `stippleFill: 'marks'` 等旧路线；新参数为 `shadingLevels`，仅接受上述五个数值。下文固定16级的历史实现说明和性能测量仍指默认档位，不代表其他级数的测量。
...
当前投影本来就是正交投影。快速模式不求球—球交线：完整原子和完整标签按球中心深度覆盖，标签不裁剪、也不检查中心是否可见。平行光下，同半径球只生成一次完整的局部明暗纹理模板，排线、点刻、规则网点均通过 SVG `use` 平移复用；不仅共享排线骨架或 PNG，量化明暗分区及其裁剪也只生成一次。不同半径分别建模板，不缩放纹理来冒充另一种半径。底色、元素图案、轮廓和标签仍按原子保留各自样式与覆盖顺序。键不会因模型含键而触发整体回退：完全埋在端点原子内的键通过几何条件判定，其余键只对附近原子做局部深度判定，生成矢量遮挡区域。坐标、物理半径和固定比例不变，但交叠球的接缝是插画式层次，并非精确三维相贯面；键—键也只排序。

API 在快速模式配投影阴影时明确报错 `Fast rendering requires castShadows=false`，不偷偷回退。键局部区域的发现步长为预览 0.5、导出 0.25 SVG 单位；已发现的边界二分细化，仍可能漏掉亚网格小片段。GUI 点刻在快速模式也使用所选级数的位图瓦片。原子模板使用局部纹理坐标，因此同规格原子的点位／网点相位随副本重复，而不是每个原子重新采样世界坐标；这是快速副本模式的明确取舍。缓存仅在当前一次渲染内共享，修改光源、级数、粗细、亮度或对比度都会生成相应的新模板。键的方向和局部遮挡各异，仍逐键处理，但只有通过遮挡检查的可见键才生成明暗纹理。

**历史测量（旧半径与旧快速纹理实现，不代表当前默认配置）**：真实 C60（含全部 90 根键）的当轮 SVG 生成中位数：默认半径排线 **209.7 → 5.0 ms**，原子缩至 0.65 倍以露出键的夹具 **238.1 → 70.2 ms**。默认半径点刻 **209.4 → 170.5 ms**、网点 **72.7 → 49.0 ms**；缩小原子的点刻反而 **222.0 → 279.3 ms**，不是所有模式都更快。默认 C60 的键全部处于端点原子并集内，缩小半径的夹具有 74 根键可见，并未移除键。计时不含浏览器绘制，原始数据与参数见 `benchmarks/fast-renderer.json`。

**本次原子模板复用对比**（当前科学半径0.75倍，16级，C60全部90根键保留，61根可见）：排线 **85.6 → 71.8 ms**，点刻 **129.2 → 87.2 ms**，规则网点 **110.6 → 78.3 ms**；SVG体积分别减少约 **51%、42%、44%**。三种纹理均为 **1份原子模板、60个引用副本**。另设无键夹具隔离原子开销，分别从9.2／41.6／20.8 ms降至1.2／1.4／1.3 ms；它不是实际完整C60的总耗时，完整场景余下主要工作仍在键的局部遮挡。数据为同进程交替5组的生成耗时中位数，不含浏览器绘制。原始数据：`benchmarks/fast-templates.json`；对比脚本：`scripts/bench-fast-templates.cjs`（需事先保留脚本注明位置的旧bundle快照）。

检查：`node test-fast-templates.cjs`、`node test-fast-renderer.mjs`、`node test-fast-ui.cjs`。历史精确／快速性能对比：`node scripts/bench-fast.mjs`。早期 `PAINTER-EXPERIMENT.md` 中的认证原型不再代表主页面快速开关的能力。
...
下文基于 pattern 的规则网点说明指 `quantizeShading: true`（精确模式默认）。设为 `false` 时，在45°规则网格上逐点输出连续半径；边缘仍收缩完整圆点，因此边界覆盖率不同于裁切 pattern，并有逐点输出成本与数量限制。精确点刻中，`true` 把局部明暗量化为同一套 16 档、圆点半径保持 `dotSize` 不变，只改变点数；`false` 保留原连续密度点刻。`stippleFill` 决定这 16 档墨怎么交付：`'marks'` 输出扁平、按「半径 + 档位」分批的圆头子路径，既无纹理瓦片也无逐原子裁剪；`'bitmap'`（API 与 GUI 默认）则在单一共享 pattern transform 下平铺烘焙的 1-bit PNG 瓦片，浏览器解码一次后直接贴图，不再逐点描边。位图路径需要已认证的 owner clip，无法认证时回退到 `'marks'`；由于瓦片载荷是固定成本，它在最小模型上反而略大。

- `textureScale`（界面「纹理粗细」）范围 0.2–2.5，三种风格的界面默认值都是 1；点刻的内部基准为原先的 0.6，规则网点为原先的 3.0，排线不变。联合调整间距和点径／线宽，尽量保持平均覆盖率。轮廓线宽独立。
- **光照先于美术处理**：物理直射量为 `D = max(0, n·L)`，投射阴影只对 D 乘 `1 - shadowStrength`；不模拟二次反射或环境光。随后才做美术基线 `B = (D + 1) / 2`、亮度／对比度和纹理映射，最后量化。美术层可以提亮物理上无直射光的区域，但这不代表增加了环境光。这样阴影跨过背光交界时不会突然解除美术偏移的衰减而出现亮边。
- `shadingBrightness` 范围 −1–1，默认 0，正值变亮；`shadingContrast` 调整明暗响应。二者位于光照之后，都不改变元素底色。控件和默认值不变；修正后的阴影／背光视觉不再与旧照明公式逐字一致。
...
- GUI 在精确与快速模式中始终传入 `quantizeShading: true`，`shadingLevels` 可选4／8／16／32／64，默认16；不发送 `hatchMode`。分级排线以所选数量的互斥明暗带裁剪固定曲面骨架，线宽分档、裁断处不收尖。规则网点覆盖率与点刻密度使用同一级数，位图点刻选择对应的预烘焙瓦片。精确排线在无法认证几何时仍保留内部 `continuous-fallback` 安全回退，移除 GUI 选择入口不等于移除这种几何回退。仅后端保留 `quantizeShading: false`：连续收尖排线、连续半径规则网点与连续密度点刻；不承诺与量化路线逐像素一致。详见[实现与历史测量](LAYERED-HATCHING.md)。
- 旧 `hatchMode: 'layered' | 'continuous'` 保留为排线兼容选项；显式冲突选项与非布尔 `quantizeShading` 仍报错。快速模式现在接受显式 `quantizeShading: true` 选择量化，`false` 使用保留的连续路线；省略时保留旧快速模式行为，GUI 始终显式传 `true`。历史测量和效果批准记录只对应当时配置，不代表所有新级数的性能或验证结果。
