/* MolWash: orthographic, depth-tested SVG color wash. No DOM or dependencies. */
import type { Scene, Primitive, Vector, DepthAt, Project, ElementColor } from './types';
export interface WashOptions { fitCurves?: boolean; }
interface WashShape { shape: Primitive; bounds: Vector; label: number; }
interface ContourNode { p: Vector; key: string; pair?: number[]; junction?: boolean; }
type Curves = Vector[][];
 ...
 */
export function buildWash(scene: Scene, depthAt: DepthAt, project: Project, scale: number, colorForElement: ElementColor, options: WashOptions = {}): string {
  var empty = '<g class="mol-wash"></g>';
  if (!Array.isArray(scene)) throw new TypeError('scene must be an array');
  if (typeof depthAt !== 'function' || typeof project !== 'function' ||
      typeof colorForElement !== 'function') throw new TypeError('Expected callback functions');
  if (!(scale > 0) || !Number.isFinite(scale)) throw new RangeError('scale must be finite and positive');
  var origin = project([0, 0, 0]);
  if (!valid2(origin)) throw new RangeError('project must return finite coordinates');
  var shapes: WashShape[] = [], colors = [''], colorIds = new Map<string, number>();
  var minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
  scene.forEach(function (s: Primitive) {
    if (!s || (s.kind !== 'sphere' && s.kind !== 'cylinder')) return;
    if (!(s.r > 0) || !Number.isFinite(s.r)) return;
    var p: Vector, q: Vector, id = 0;
    if (s.kind === 'sphere') {
      if (!valid3(s.c)) return;
      p = q = project(s.c);
      var color = colorForElement(s.element);
      if (color != null && color !== '') {
        color = String(color).trim().toLowerCase();
        if (color === 'white' || color === '#fff') color = '#ffffff';
        if (!/^#[0-9a-f]{6}$/.test(color)) throw new TypeError('Element colors must be #rrggbb');
        if (color !== '#ffffff') {
          if (!colorIds.has(color)) { colorIds.set(color, colors.length); colors.push(color); }
          id = colorIds.get(color)!;
        }
      }
    } else {
      if (!valid3(s.a) || !valid3(s.u) || !(s.length >= 0) || !Number.isFinite(s.length)) return;
      p = project(s.a);
      q = project([s.a[0] + s.u[0] * s.length, s.a[1] + s.u[1] * s.length,
        s.a[2] + s.u[2] * s.length]);
    }
    if (!valid2(p) || !valid2(q)) return;
    var r = s.r * scale;
    var b = [Math.min(p[0], q[0]) - r, Math.min(p[1], q[1]) - r,
      Math.max(p[0], q[0]) + r, Math.max(p[1], q[1]) + r];
    if (!b.every(Number.isFinite)) throw new RangeError('Projected bounds overflow');
  ...
  if (maxX === -Infinity || colors.length === 1) return empty;
  var width = maxX - minX, height = maxY - minY;
  if (!Number.isFinite(width) || !Number.isFinite(height)) throw new RangeError('Projected extent overflow');
  // Also bound skinny, extremely long viewports; padding covers outer edges.
  var step = Math.max(1, Math.sqrt(width) * Math.sqrt(height) / 1998,
    width / 3990000, height / 3990000);
  var nx = Math.ceil(width / step) + 2, ny = Math.ceil(height / step) + 2;
  while (nx * ny > 4000000) {
    step *= 1.1; nx = Math.ceil(width / step) + 2; ny = Math.ceil(height / step) + 2;
  }
    ...
    for (var y = top; y <= bottom; y++) {
      ...
      for (var x = left; x <= right; x++) {
        ...
        if (Number.isFinite(z) && (z > depths[i] || (z === depths[i] && item.label === 0))) {
          depths[i] = z; labels[i] = item.label;
        }
      }
    }
  });
  ...
  var stride = nx + 1, graphs = new Map<number, Map<number, number>>();
  function edge(label: number, x: number, y: number, direction: number) {
    var graph = graphs.get(label);
    if (!graph) { graph = new Map(); graphs.set(label, graph); }
    var v = y * stride + x;
  ...
  for (var y = 0; y < ny; y++) {
    for (var x = 0; x < nx; x++) {
      ...
      if (!label) continue;
      if (!y || labels[i - nx] !== label) edge(label, x, y, 0);
      if (x === nx - 1 || labels[i + 1] !== label) edge(label, x + 1, y, 1);
      if (y === ny - 1 || labels[i + nx] !== label) edge(label, x + 1, y + 1, 2);
      if (!x || labels[i - 1] !== label) edge(label, x, y + 1, 3);
    }
  }
  ...
  function owner(p: Vector) {
    ...
    for (var j = 0; j < shapes.length; j++) {
      ...
      if (Number.isFinite(z) && (z > best || (z === best && item.label === 0))) {
        best = z; label = item.label;
      }
    }
    return label;
  }
  function bisect(a: Vector, b: Vector, label: number) {
    for (var k = 0; k < 32 && distance(a, b) > 0.0001; k++) {
      var m = mix(a, b, 0.5);
      if (owner(m) === label) a = m; else b = m;
    }
    return mix(a, b, 0.5);
  }
  var output = ['<g class="mol-wash" stroke="none">'];
  var delta = [1, stride, -1, -stride], crossings = new Map<string, ContourNode>(), junctions = new Map<number, ContourNode | null>(), fitted = new Map<string, Curves>();
  function center(x: number, y: number) { return [x0 + (x + 0.5) * step, y0 + (y + 0.5) * step]; }
  function crossing(v: number, direction: number): ContourNode {
    var end = v + delta[direction], key = Math.min(v, end) + ':' + Math.max(v, end);
    if (crossings.has(key)) return crossings.get(key)!;
    var x = v % stride, y = Math.floor(v / stride), a!: Vector, b!: Vector;
    if (direction === 0) { a = center(x, y - 1); b = center(x, y); }
    if (direction === 1) { a = center(x - 1, y); b = center(x, y); }
    if (direction === 2) { a = center(x - 1, y - 1); b = center(x - 1, y); }
    if (direction === 3) { a = center(x - 1, y - 1); b = center(x, y - 1); }
    ...
    for (var probe = 1; probe < 8; probe++) {
      ...
      if (middleLabel !== la && middleLabel !== lb) {
        ...
        if (ja && jb && distance(jb.p, boundary) < distance(ja.p, boundary)) nearest = jb;
        if (nearest) boundary = nearest.p;
        break;
      }
    }
    var point = { p: boundary, key: key, pair: [Math.min(la, lb), Math.max(la, lb)] };
    crossings.set(key, point); return point;
  }
  function junction(v: number): ContourNode | null {
    if (junctions.has(v)) return junctions.get(v)!;
    var x = v % stride, y = Math.floor(v / stride);
    var corners = [center(x - 1, y - 1), center(x, y - 1), center(x, y), center(x - 1, y)];
    var ls = corners.map(owner), tri: Vector[] | null = null;
    for (var a = 0; a < 4 && !tri; a++) for (var b = a + 1; b < 4 && !tri; b++)
      for (var c = b + 1; c < 4; c++) if (ls[a] !== ls[b] && ls[b] !== ls[c] && ls[a] !== ls[c]) {
        tri = [corners[a], corners[b], corners[c]]; break;
      }
    if (!tri) { junctions.set(v, null); return null; }
    ...
    for (var it = 0; it < 16 && cells[0][2] > 0.0001; it++) {
      ...
      cells.forEach(function (cell) {
        ...
        for (var yy = 0; yy <= 16; yy++) for (var xx = 0; xx <= 16; xx++) samples.push(owner([bx + xx * h, by + yy * h]));
        for (var yy = 0; yy < 16; yy++) for (var xx = 0; xx < 16; xx++) {
          var at = yy * 17 + xx;
          var ids = new Set([samples[at], samples[at + 1], samples[at + 17], samples[at + 18]]);
          if (ids.size >= 3) found.push([bx + xx * h, by + yy * h, h]);
        }
      });
    ...
    junctions.set(v, result); return result;
  }
  function refine(a: Vector, b: Vector, pair: number[], depth: number, result: Vector[]) {
    var len = distance(a, b);
    if (len < 0.025 || depth > 12) { result.push(b); return; }
    var m = mix(a, b, 0.5), n = [(b[1] - a[1]) / len, (a[0] - b[0]) / len];
    var span = Math.max(0.02, len * 0.6), lo!: Vector, hi!: Vector, l!: number, h!: number;
    for (var k = 0; k < 5; k++) {
      lo = add(m, n, -span); hi = add(m, n, span); l = owner(lo); h = owner(hi);
      if (l !== h && pair.indexOf(l) >= 0 && pair.indexOf(h) >= 0) break;
      span *= 0.5;
    }
    var thirdRegion = false;
    if (l !== h && pair.indexOf(l) >= 0 && pair.indexOf(h) >= 0) {
      for (var probe = 1; probe < 8; probe++) if (pair.indexOf(owner(mix(lo, hi, probe / 8))) < 0) thirdRegion = true;
    }
    if (thirdRegion || l === h || pair.indexOf(l) < 0 || pair.indexOf(h) < 0) {
      ...
      var previous = add(m, n, -len), previousLabel = owner(previous), best = Infinity, bracket: [Vector, Vector, number] | null = null;
      for (var scan = 1; scan <= 128; scan++) {
        ...
        if (candidateLabel !== previousLabel && pair.indexOf(candidateLabel) >= 0 && pair.indexOf(previousLabel) >= 0) {
          ...
          if (score < best) { best = score; bracket = [previous, candidate, previousLabel]; }
        }
        previous = candidate; previousLabel = candidateLabel;
      }
      if (!bracket) { result.push(b); return; }
      lo = bracket[0]; hi = bracket[1]; l = bracket[2];
    }
    var q = bisect(lo, hi, l);
    if (distance(m, q) > 0.003 || len > 0.5) {
      refine(a, q, pair, depth + 1, result); refine(q, b, pair, depth + 1, result);
    } else result.push(b);
  }
  function chain(nodes: ContourNode[], pair: number[]): Curves {
    ...
    if (!curves) {
      ...
      for (var j = 1; j < nodes.length; j++) refine(nodes[j - 1].p, nodes[j].p, pair, 0, samples);
    ...
    if (reverse) curves = curves.slice().reverse().map(function (c) { return c.slice().reverse(); });
    return curves;
  }
  function contour(nodes: ContourNode[]) {
    var cuts = [];
    for (var k = 0; k < nodes.length; k++) if (nodes[k].junction) cuts.push(k);
    ...
    if (!cuts.length) {
      var first = 0;
      for (var k = 1; k < nodes.length; k++) if (nodes[k].key < nodes[first].key) first = k;
      var far = first;
      for (var k = 0; k < nodes.length; k++) if (distance(nodes[k].p, nodes[first].p) > distance(nodes[far].p, nodes[first].p)) far = k;
    ...
    for (var k = 0; k < cuts.length; k++) {
      ...
      while (at !== end) { run.push(nodes[at]); at = (at + 1) % nodes.length; }
      run.push(nodes[end]);
  ...
  graphs.forEach(function (graph, label) {
    ...
    graph.forEach(function (_, start) {
      while (graph.get(start)) {
        ...
        while (!(mask & (1 << direction))) direction++;
        var v = start, points: ContourNode[] = [];
        do {
          var jp = junction(v);
          if (jp) points.push(jp);
          ...
          var nextDirection = -1;
          for (var k = 0; k < 4; k++) {
            if (available & (1 << choices[k])) { nextDirection = choices[k]; break; }
          }
          if (nextDirection < 0) throw new Error('Open wash contour');
          v = next; direction = nextDirection;
        } while (true);
        if (points.length >= 3) loops.push(contour(points));
      }
    });
    if (loops.length) output.push('<path fill="' + colors[label] + '" fill-rule="evenodd" d="' + loops.join('') + '"/>');
  });
  output.push('</g>');
  return output.join('');
}
function coord(p: Vector) { return format(p[0]) + ' ' + format(p[1]); }
function distance(a: Vector, b: Vector) { return Math.hypot(a[0] - b[0], a[1] - b[1]); }
function mix(a: Vector, b: Vector, t: number) { return [a[0] * (1 - t) + b[0] * t, a[1] * (1 - t) + b[1] * t]; }
function add(a: Vector, b: Vector, s: number) { return [a[0] + b[0] * s, a[1] + b[1] * s]; }
function unit(a: Vector, b: Vector) { var d = distance(a, b); return d ? [(b[0] - a[0]) / d, (b[1] - a[1]) / d] : [1, 0]; }
function dot(a: Vector, b: Vector) { return a[0] * b[0] + a[1] * b[1]; }
function bezier(c: Vector[], t: number) { var a = mix(c[0], c[1], t), b = mix(c[1], c[2], t), d = mix(c[2], c[3], t); return mix(mix(a, b, t), mix(b, d, t), t); }
...
export function fitContour(input: Vector[], tolerance: number): Curves {
  var p = input.filter(function (q, i) { return !i || i === input.length - 1 || distance(q, input[i - 1]) > 0.0002; });
  while (p.length > 2 && distance(p[p.length - 2], p[p.length - 1]) <= 0.0002) p.splice(p.length - 2, 1);
  var result: Curves = [];
  if (p.length < 2) return result;
  function fit(lo: number, hi: number, left: Vector, right: Vector, depth: number) {
    var count = hi - lo, length = 0, u = [0];
    for (var i = lo + 1; i <= hi; i++) { length += distance(p[i - 1], p[i]); u.push(length); }
    if (count === 1) { result.push([p[lo], p[hi]]); return; }
    u = u.map(function (v) { return v / length; });
    var split = lo + Math.floor(count / 2), curve: Vector[];
    for (var iteration = 0; iteration < 5; iteration++) {
      ...
      for (var j = 0; j < count; j++) {
        ...
        if (err > error) { error = err; split = Math.max(lo + 1, Math.min(hi - 1, lo + j)); }
      }
      if (error <= tolerance) { result.push(curve); return; }
      if (iteration === 4 || error > tolerance * 8) break;
      ...
      for (var j = 1; j < count; j++) {
        ...
        if (!(next > updated[j - 1] && next < 1)) valid = false;
        updated.push(next);
      }
      if (!valid) break;
      updated.push(1); u = updated;
    }
    if (depth > 32) { for (var i = lo + 1; i <= hi; i++) result.push([p[i - 1], p[i]]); return; }
  ...
  for (var i = 1; i < p.length - 1; i++) {
    var a = i - 1, b = i + 1;
    while (a > 0 && distance(p[a], p[i]) < 0.04) a--;
    while (b < p.length - 1 && distance(p[b], p[i]) < 0.04) b++;
    if (dot(unit(p[a], p[i]), unit(p[i], p[b])) < 0.8 && i - cuts[cuts.length - 1] > 1) cuts.push(i);
  }
...
function valid2(p: Vector) { return p && Number.isFinite(p[0]) && Number.isFinite(p[1]); }
function valid3(p: Vector) { return valid2(p) && Number.isFinite(p[2]); }
function format(n: number) { return String(Math.round(n * 1000) / 1000); }
