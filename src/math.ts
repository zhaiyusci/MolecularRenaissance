...
export const add = (a: readonly number[], b: readonly number[]): Vector => a.map((v,i)=>v+b[i]);
export const sub = (a: readonly number[], b: readonly number[]): Vector => a.map((v,i)=>v-b[i]);
export const mul = (a: readonly number[], s: number): Vector => a.map(v=>v*s);
export const dot = (a: readonly number[], b: readonly number[]): number => a.reduce((s,v,i)=>s+v*b[i],0);
