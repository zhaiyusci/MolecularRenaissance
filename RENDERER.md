...
- `npm test`：先构建，再运行**闸门道**——其中的脚本都必须保持在秒级，不覆盖已有样张；单独运行 `node test.cjs` 仍可重新生成默认样张。
- `npm run test:heavy`：运行穷举扫掠（旋转表、色调与数值扫掠）等不适合放进闸门道的重测试；`npm run test:all` 两道都跑。闸门道与重扫掠分开，是因为没人愿意等的闸门会失效：曾经有一套十分钟的测试在合并时带着九个红色用例而无人察觉。

### 测试规模：按状态数，不按次数量

用例数量不是信心来源，**覆盖才是**；对连续输入撒大量均匀样本，绝大部分只是重复验证同一个分支，而且失败时无法定位。已固化的规则：

- **连续域**用边界采样加二分定位，不用密集撒点。`test-surface-tones.cjs` 原为每例 23×23=529 个均匀点（保底分辨率仅约 2px 格距），改为离轴射线二分求交加 ±0.7px 采样后，边界定位精度提高约 2.8 倍，点数降到 1/12.8；`test-svg-numbers.cjs` 从 2,106,018 次比对降到 4,343，却**新发现并补上了原扫描一直缺失的精度极限与 `[1e20,1e21)` 平写区间**。
- **有限集合要全枚举**，不要抽样：有限域里每个元素都是独立数据点，没有可省的相邻冗余。1000 条尾表因此逐条全测。
- **先量状态数，再定用例数。** `test-c60-rotations.cjs` 用每视角的 `(curveCount | 有轮廓原子数 | 精确重合原子对数)` 测出 466 个视角只访问 13 个 regime，于是降到 27 个覆盖全部 regime 的边界姿态（31.5 s → 1.5 s）；又证明 `Ry(180°)` 保持 C60 点集，`yaw ≡ yaw+180` 使另一半视角零信息，直接砍掉。
- **每个文件加命名常量形式的用例上限**，超过即失败，防止规模悄悄回升。
- **瘦身后必须证明测试仍有牙齿**：在内存中注入变异（改模块加载或 `fs.readFileSync` 的返回值，**不写盘**），逐一确认退出码非零，并如实报告**没抓到**的变异。若某个变异旧测试同样抓不到，那是既有盲区而非本次削弱；若只有新测试抓不到，说明该性质已失去保护，必须恢复采样，哪怕付出运行时代价。
