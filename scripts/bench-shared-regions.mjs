...
for(const model of ['sphere','ethanol','c60'])for(const castShadows of [false,true])for(const shadingMode of ['hatch','stipple']){
  ...
  if(before){
    ...
    for(let k=0;k<2;k++)for(let side=0;side<2;side++)engines[side](models[side],options);
    for(let k=0;k<5;k++)for(const side of (k%2?[1,0]:[0,1])){
      const start=performance.now(),svg=engines[side](models[side],options);times[side].push(performance.now()-start);bytes[side]=svg.length;
    }
...
writeFileSync('benchmarks/regions-'+(paired?'paired':capture?'before':'after')+'.json',JSON.stringify({note:paired?'Core SVG only, no browser paint. Same process, two warmups each; 5 alternating old/new pairs per model/settings, medians. Scale 60, textureScale 1, preview, color wash.':'Core SVG generation only; fixed scale 60, textureScale 1, preview, color wash; warmup + median of 3, no browser paint.',rows},null,2)+'\n');
